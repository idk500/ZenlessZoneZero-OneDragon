...
class PluginImportService:
    ...
    def plugins_dir(self) -> Path:
        ...
        优先取上下文插件目录列表中标记为第三方的目录；
        目录尚不存在（首次导入插件）时，按框架同样的规则推导项目根下的 plugins 目录。
        """
        for dir_path, source in self.ctx.application_plugin_dirs:
            if source == PluginSource.THIRD_PARTY:
                return dir_path
        src_dir = file_utils.find_src_dir(Path(__file__))
        if src_dir is not None:
            return src_dir.parent / 'plugins'
        # 兜底：本文件位于 src/one_dragon/base/operation/application/ 下，上溯 5 级为项目根
