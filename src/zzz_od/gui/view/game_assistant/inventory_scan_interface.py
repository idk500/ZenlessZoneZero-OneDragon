...
class InventoryScanInterface(AppRunInterface):
    ...
    def on_interface_shown(self) -> None:
        ...
        # 勾选状态回读持久化的配置
        self.scan_drive_disk_check.setChecked(self.config.scan_drive_disk)
        self.scan_wengine_check.setChecked(self.config.scan_wengine)
        self.scan_agent_check.setChecked(self.config.scan_agent)

    def _on_start_clicked(self) -> None:
        """启动应用前保存扫描目标选择"""
        if hasattr(self, 'config'):
            self.config.scan_drive_disk = self.scan_drive_disk_check.isChecked()
            self.config.scan_wengine = self.scan_wengine_check.isChecked()
            self.config.scan_agent = self.scan_agent_check.isChecked()
            log.info(
                f"扫描目标: drive_disk={self.config.scan_drive_disk}, "
                f"wengine={self.config.scan_wengine}, agent={self.config.scan_agent}"
            )
